...
def _is_gender_match(user, candidate):
    wanted = str(user.get("interested_in") or "").lower()
    candidate_gender = str(candidate.get("gender") or "").lower()

    if not wanted or wanted in {"any", "all", "everyone"}:
        return True

    return wanted == candidate_gender
...
def _compatibility_score(user, candidate, preferences):
    ...
    if _is_gender_match(user, candidate):
        score += 25
...
def get_recommendations(user_id, limit=10):
    ...
    for candidate_row in candidates:
        ...
        if not _is_gender_match(user, candidate):
            continue
